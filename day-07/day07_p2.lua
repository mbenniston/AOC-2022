...
CurrentlyUsed = GetDirectorySize(FileSystem)
Required = 30000000
FileSystemMax = 70000000
AlreadyFree = FileSystemMax - CurrentlyUsed
NeedToFree = Required - AlreadyFree


function GetSmallestRequiredDirectory(directory)
    local directorySum = GetDirectorySize(directory)

    local smallest = directorySum

    for index, value in pairs(directory) do
        if type(value) == "table" then
            local smallestSubDirectory = GetSmallestRequiredDirectory(value)

            if smallestSubDirectory >= NeedToFree and smallestSubDirectory < smallest then
                smallest = smallestSubDirectory
            end
        end
    end
...
print(GetSmallestRequiredDirectory(FileSystem))
