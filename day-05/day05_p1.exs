defmodule CrateMoving do
  # Moves a single crate from the from slot to the to slot
  def move_single_crate(state, fromSlot, toSlot) do
    Enum.with_index(state)
    |> Enum.map(fn {slots, index} ->
      cond do
        index == fromSlot ->
          List.delete_at(slots, 0)

        index == toSlot ->
          List.insert_at(slots, 0, List.first(Enum.at(state, fromSlot)))
  ...
  # Run move crates multiple times feeding the output of the previous move
  # into the next
  def move_crates(state, numCrates, fromSlot, toSlot) do
    if numCrates == 0 do
      state
    else
      next_state = move_single_crate(state, fromSlot, toSlot)
      move_crates(next_state, numCrates - 1, fromSlot, toSlot)
    end
  end
...
IO.puts(
  ...
  |> Enum.reduce("", fn crate, acc -> acc <> crate end)
)
